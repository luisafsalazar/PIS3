...
if __name__ == "__main__":
    ...
    # Configurar la semilla global
    seed = config.get('seed', 42)
    set_global_seed(seed)
    
    # Determinar qué configuración usar
    if config.get('is_brats2021', False):
        config = config.trainer.brats2021
        data_flag = "brats2021"
        is_HepaticVessel = False
    elif config.get('is_brain2019', False):
        config = config.trainer.brain2019
        data_flag = "brain2019"
        is_HepaticVessel = False
    else:
        config = config.get('hepatic_vessel2021', config)
        data_flag = "hepatic_vessel2021"
        is_HepaticVessel = True

    logging_dir = os.path.join(os.getcwd(), "logs", datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    ...
    if config.get('resume', False):
        model, starting_epoch, step, val_step = utils.resume_train_state(
            model, "{}".format(config.trainer.finetune.checkpoint), train_loader, accelerator
        )
    ...
    for epoch in range(starting_epoch, config.trainer.num_epochs):
        ...
        if mean_acc > best_acc:
            accelerator.save_state(
                output_dir=os.path.join(os.getcwd(), "model_store", config.trainer.finetune.checkpoint, "best")
            )
            best_acc = mean_acc
            best_class = batch_acc
            best_epoch = epoch
        accelerator.save_state(
            output_dir=os.path.join(os.getcwd(), "model_store", config.trainer.finetune.checkpoint, f"epoch_{epoch}")
        )
